...
def _ensure_can_view_lease(current: dict, lease: models.Lease, db: Session):
    ...
    if role == "tenant":
        if not tenant or tenant.id != sub:
    ...
    if role in {"manager", "property_manager"}:
        # current implementation allows manager org/staff to view
        return

    raise HTTPException(status_code=403, detail="Forbidden")


def _build_lease_pdf_bytes(
    lease: models.Lease,
    ...
    terms_text = getattr(lease, "terms_text", None) or _default_terms_text(
...
def read_lease(
    ...
    _ensure_can_view_lease(current, lease, db)
...
def lease_pdf_dot(
    ...
    return _lease_pdf_common(lease_id, db, current)
