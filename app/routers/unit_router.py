...
def update_unit(unit_id: int, unit_update: schemas.UnitUpdate, db: Session = Depends(get_db)):
    unit = crud.unit.update_unit(db, unit_id, unit_update)
    if not unit:
        raise HTTPException(status_code=404, detail="Unit not found")
    return unit
...
def delete_unit(unit_id: int, db: Session = Depends(get_db)):
    unit = crud.unit.delete_unit(db, unit_id)
    if not unit:
        raise HTTPException(status_code=404, detail="Unit not found")
...
def list_units_by_property(property_id: int, db: Session = Depends(get_db)):
    units = crud.unit.get_units_by_property(db, property_id)
    if not units:
...
def list_available_units(property_id: int = None, db: Session = Depends(get_db)):
    return crud.unit.get_available_units(db, property_id)
