...
def create_payment(payment: schemas.PaymentCreate, db: Session = Depends(get_db)):
    return crud.payments.create_payment(db, payment)

# Get Payment by ID
@router.get("/{payment_id}", response_model=schemas.PaymentOut)
def read_payment(payment_id: int, db: Session = Depends(get_db)):
    db_payment = crud.payments.get_payment(db, payment_id)
    if not db_payment:
...
def list_payments(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return crud.payments.get_payments(db, skip=skip, limit=limit)
...
def update_payment(payment_id: int, update_data: schemas.PaymentUpdate, db: Session = Depends(get_db)):
    db_payment = crud.payments.update_payment(db, payment_id, update_data)
    if not db_payment:
        raise HTTPException(status_code=404, detail="Payment not found")
    return db_payment
...
def delete_payment(payment_id: int, db: Session = Depends(get_db)):
    success = crud.payments.delete_payment(db, payment_id)
    if not success:
        raise HTTPException(status_code=404, detail="Payment not found")
...
def payments_by_tenant(tenant_id: int, db: Session = Depends(get_db)):
    return crud.payments.get_payments_by_tenant(db, tenant_id)
...
def payments_by_unit(unit_id: int, db: Session = Depends(get_db)):
    return crud.payments.get_payments_by_unit(db, unit_id)
...
def payments_by_lease(lease_id: int, db: Session = Depends(get_db)):
    return crud.payments.get_payments_by_lease(db, lease_id)
...
def payments_by_date_range(
    ...
    return crud.payments.get_payments_by_date_range(db, start_date, end_date)
